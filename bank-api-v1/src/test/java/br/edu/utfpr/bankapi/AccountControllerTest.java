...
public class AccountControllerTest {
    ...
    void deveriaSalvarConta() throws Exception {
        // ARRANGE
        String json = "{ \"name\": \"Amanda Faria\", \"number\": 12345, \"balance\": 0.0, \"specialLimit\": 0.0 }";
    ...
    void deveriaAtualizarConta() throws Exception {
        // ARRANGE
        long accountId = 1L;
        String json = "{ \"name\": \"Amanda Faria\", \"number\": 12345, \"balance\": 1500.0, \"specialLimit\": 0.0 }";

        // ACT & ASSERT
        mockMvc.perform(MockMvcRequestBuilders.put("/account/" + accountId)
                        .content(json).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.name", Matchers.is("Amanda Faria")))
                .andExpect(jsonPath("$.number", Matchers.is((int) 12345)))
